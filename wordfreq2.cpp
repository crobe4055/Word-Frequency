...
void insert(binary_tree_node<frequency<std::string> >* root,
	...
	if (root->data() > new_node) {
		...
		root->data().operator ++();// if (root->data() == new_node), If a word is seen again, increment the count.
	}

}

void insertFreq(binary_tree_node<frequency<std::string> >* root,
		binary_tree_node<frequency<std::string> >* new_node) { // This is pretty much the same as insert, but it compares the frequency count instead of alphabetical order.

	if (root->data().count() < new_node->data().count()) { //  IF roots data count is less then the data count of the new node.
		if (root->left() == NULL) {
			root->set_left(
					new binary_tree_node<frequency<std::string> >(
							new_node->data()));
...
void alphaToFreq(binary_tree_node<frequency<std::string> > *head_ptr,
		binary_tree_node<frequency<std::string> > *alpha_root,
		binary_tree_node<frequency<std::string> > * freq_root) {
	if (alpha_root == NULL) // If the root of the alphabetical tree is NULL return.
		return;

	if (alpha_root != head_ptr) // Prevents freq tree from having root node twice
		{insertFreq(freq_root, alpha_root);}
